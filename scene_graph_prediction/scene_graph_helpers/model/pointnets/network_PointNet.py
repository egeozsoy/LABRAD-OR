...
class PointNetRelCls(BaseNetwork):
    ...
    def __init__(self, k=2, in_size=1024, batch_norm=True, drop_out=True,
                 init_weights=True, image_embedding_size=None, n_object_types=None, history_embedding_size=None):
        ...
        if history_embedding_size is None:
            history_embedding_size = 0
        self.fc1 = nn.Linear(in_size, 512)
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256 + image_embedding_size + n_object_types * 2 + history_embedding_size, k)
        if self.use_drop_out:
            self.dropout = nn.Dropout(p=0.3)
        if batch_norm:
            self.bn1 = nn.BatchNorm1d(512)
    ...
    def forward(self, x, relation_objects_one_hot=None, image_embeddings=None, history_embeddings=None):
        if history_embeddings is not None:
            history_embeddings = history_embeddings.unsqueeze(0).repeat(len(x), 1)
        x = self.fc1(x)
        if self.use_bn:
            x = self.bn1(x)
        x = self.relu(x)
        ...
        if history_embeddings is not None:  # Also late fusion
            x = torch.cat([x, history_embeddings], dim=1)
        x = self.fc3(x)
